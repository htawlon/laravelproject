...
class PostController extends Controller
{
    ...
    public function postUpdatePost(Request $request){
        ...
        $post->update();
        return redirect()->route('posts.posts')->with("info", 'The selected have been update.');
    }

    public function editPost($id){
        $cats=Category::get();
        $post=Post::whereId($id)->firstOrFail();
        return view('posts.edit_post')->with(['post'=>$post , 'cats'=>$cats]);
    ...
    public function getDropPost($id){
        ...
        Storage::disk('posts')->delete($post->image);
        $post->delete();
        return redirect()->back()->with('info','he selected post have been delete.');
    }
    ...
    public function addPost(){
        $cats=Category::get();
        return view('posts.addpost')->with(['cats'=>$cats]);
    }
}
